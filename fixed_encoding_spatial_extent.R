...
# 2. Load sites data with encoding handling
cat("读取sites数据...\n")
...
tryCatch({
  ...
  tryCatch({
    # 方法2: 使用GBK编码
    sites <- st_read("D:/Yejin/hs/huangshi/data/raw_data/surveypoints.shp", 
                     options = c("ENCODING=GBK"))
    cat("✓ 使用GBK编码成功读取\n")
    
  }, error = function(e2) {
    cat("GBK编码失败，尝试CP936编码...\n")
    
    tryCatch({
      # 方法3: 使用CP936编码
      sites <- st_read("D:/Yejin/hs/huangshi/data/raw_data/surveypoints.shp", 
                       options = c("ENCODING=CP936"))
      cat("✓ 使用CP936编码成功读取\n")
      ...
      tryCatch({
        ...
        # 方法5: 使用terra包
        sites_vect <- vect("D:/Yejin/hs/huangshi/data/raw_data/surveypoints.shp")
        sites <- st_as_sf(sites_vect)
        cat("✓ 使用terra包成功读取\n")
      })
    })
  })
...
# 检查读取结果
if(!exists("sites") || is.null(sites)) {
  stop("无法读取sites数据，请检查文件路径和编码")
}

cat("Sites数据读取成功，共", nrow(sites), "个点\n")
cat("字段名称:", names(sites), "\n")
...
# 4. Crop ALL rasters to the sites extent (including lcp_density)
cat("裁剪所有栅格到sites范围...\n")
...
# 5. Verify all extents are the same
cat("验证所有栅格的空间范围...\n")
...
# 6. Create a function to plot with consistent extent
...
# 7. Plot all rasters with consistent extent
cat("生成一致空间范围的图像...\n")
...
# 8. Optional: Save the cropped rasters for future use
cat("保存裁剪后的栅格...\n")
